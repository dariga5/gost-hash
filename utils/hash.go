...
func GetHashFromString(str string, hashtype int) {

}

func gost_xor(vecA [64]char, vecB [64]char) [64]char {
	var result [64]char
...
func gost_plus(vecA [64]char, vecB [64]char) [64]char {
	var result [64]char
	...
	for i >= 0 {
		sum := int(vecA[i]) + int(vecB[i]) + remaim
		if sum > 256 {
			remaim = (sum >> 8)
		} else {
			remaim = 0
...
func gost_x(arrK [64]char, arr [64]char) [64]char {

	return gost_xor(arrK, arr)

}
func p(arr [64]char) [64]char {
	var result [64]char
	tau := [64]char{
		0, 8, 16, 24, 32, 40, 48, 56,
		1, 9, 17, 25, 33, 41, 19, 57,
...
func gost_lps(arr [64]char) [64]char {
	var result [64]char
...
func e(k [64]char, m [64]char) [64]char {
	var result [64]char
	c := [768]char{
	...
	for i := 0; i < 12; i++ {
		result = gost_lps(result)

		buff := [64]char(c[(i * 64):((i * 64) + 64)])

		k = gost_xor(tmp, buff)
		k = gost_lps(k)
		result = gost_x(k, result)
	}
	return result
}

func g(N [64]char, h [64]char, m [64]char) [64]char {
	var result [64]char

	result = gost_xor(h, N)
	result = gost_lps(result)
	result = e(result, m)
	result = gost_xor(result, h)
	result = gost_xor(result, h)
